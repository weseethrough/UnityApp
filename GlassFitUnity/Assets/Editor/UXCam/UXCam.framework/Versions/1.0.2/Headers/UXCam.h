...
/* Call this method from applicationDidFinishLaunching. This will start the UXCam application, ping the server, get the settings configurations and start capturing the data according to the configuration. */
...
/* Starts a new session after the stopApplicationAndUploadData method has been called. Happens automatically when the app returns from background. */
+ (void) restartSession;
...
/* UXCam captures the view controller name automatically but in cases where it doesn’t (such as in OpenGL applications) or you would like to set a different unique name, use this function. */
+ (void) tagScreenName:(NSString*)screenName;

/* UXCam uses a unique number to tag a device. You can tag a device allowing you to search for it on the dashboard and review their sessions further. */
