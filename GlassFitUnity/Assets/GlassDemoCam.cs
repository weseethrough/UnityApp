...
public class GlassDemoCam : MonoBehaviour {
	...
	{		
		...
		if((GUI.RepeatButton(new Rect(200, 0, 400, 250), "", GUIStyle.none) || sc.GetComponent<Scriptholder>().isTapped))
		{ 
	...
	void Update () {
		...
		Quaternion newOffset = Quaternion.Inverse(offsetFromStart) * Platform.Instance.getGyroDroidQuaternion();
